...
app.get('/datasoal', async (req, res, next) => {
        fetch(encodeURI(`https://pastebin.com/raw/cq71EX7f`))
        .then(response => response.json())
        .then(data => {
        hdata = data.result;
        fdata = hdata[Math.floor(Math.random() * hdata.length)];
        var result = fdata;
             res.json({
                 status : true,
                 result,
...
app.get('/waifu', async (req, res, next) => {
       fetch(encodeURI(`https://raw.githubusercontent.com/MikuWhatsBOT/private/main/test.json`))
        .then(response => response.json())
        .then(data => {
...
app.get('/datasoalp', async (req, res, next) => {
        fetch(encodeURI(`https://pastebin.com/raw/UBMvj8cv`))
        .then(response => response.json())
        .then(data => {
        hdata = data.result;
        fdata = hdata[Math.floor(Math.random() * hdata.length)];
